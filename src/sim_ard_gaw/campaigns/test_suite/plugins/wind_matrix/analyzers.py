...
class WindMatrixAnalyzer(Analyzer):
    ...
    name: str = "wind_matrix_legacy_analysis"
    ...
    def _analyze(self, case: TestCase, ctx: AttemptContext) -> AnalysisResult:
        ...
        plugin_fields = {
            ...
            "run_alias": run_alias,
            "start_time_utc": ctx.extra.get("legacy_start_time_utc") or end_time,
        ...
        return AnalysisResult(
            ...
            summary={
                ...
                "legacy_success_class": success_class,
                "legacy_analysis_status": analysis_status,
            },
            output_paths=[dest_bin],
            error=None if legacy_analysis_succeeded(analysis_status) else analysis_status,
        )
...
class WindMatrixVerdictPolicy(VerdictPolicy):
    def classify(
        ...
        if analysis_results:
            ...
            legacy_status = str(summary.get("legacy_status") or legacy_status)
            analysis_status = summary.get("legacy_analysis_status")
        verdict_class = _VERDICT_BY_LEGACY_STATUS.get(
            legacy_status, VerdictClass.FAILED_RETRYABLE,
        )
        return Verdict(
            klass=verdict_class,
            reason=legacy_status,
            retryable=verdict_class == VerdictClass.FAILED_RETRYABLE,
            requires_analysis=legacy_status in {
                "success_full",
                "success_square_only",
                "failed_analysis",
            },
...
def _legacy_status_from_monitor(
    state: dict[str, Any],
    *,
