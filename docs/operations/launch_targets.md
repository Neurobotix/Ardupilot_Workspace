...
| `plane-gps` | `config/vehicles/plane_base.parm` -> `config/overlays/plane_gps.parm` by default, or the explicit GPS envelope stack from `SIM_ARD_GAW_GPS_PARAM_FILES`. | Local override **excluded unconditionally** (dedicated `build_plane_gps_param_args`, exclusion printed); wipes EEPROM. Baseline pairs with `gazebo-plane-gps` (`mini_talon_gps_runway.sdf`, east-facing sensor-neutral model). The named `fast_cruise_18mps` envelope appends `config/overlays/plane_gps_airspeed_fast_cruise_18mps.parm` and pairs with `gazebo-plane-gps-airspeed` (`mini_talon_gps_airspeed_runway.sdf`, east-facing model with Gazebo JSON airspeed). Corrected fast-envelope live verification remains pending. |
