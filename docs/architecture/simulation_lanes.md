...
| GPS failure behavior | Behavior characterization under degraded/corrupted GPS signal (drift/glitch/denial/jamming); EKF innovation-gate "knee"; `test_suite` plugin lane | `plane-gps` + `gazebo-plane-gps` (dedicated identities; base+`plane_gps.parm` only, no airspeed overlay, no local override) | No | Phase 0 design lock accepted 2026-07-06; dedicated launch targets added structurally 2026-07-13 (not live-smoke verified); plugin and live runs open. See `docs/architecture/gps_failure_lane.md` |
