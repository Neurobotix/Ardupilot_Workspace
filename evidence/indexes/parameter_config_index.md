...
| --- | --- | --- | --- | --- | --- |
| `config/vehicles/plane_base.parm` | Sensor-neutral Mini Talon base with generic `AIRSPEED_*` defaults. | vehicle base | yes | `410226094248709c7660ee51517b07a9077314d1d15a774b8453794c343c09c4` | First file for `plane`, `plane-cte` / `plane-airspeed`, `plane-lidar`, `plane-staircase`, `plane-airspeed-lidar`, `plane-altitude-wind`, the `gps_failure` lane, and current CTE campaign callers. |
...
| `gps_failure` | `plane_base.parm` -> `plane_gps.parm` | Plugin default stack; caller-supplied stacks override it. Static/no-SITL only; no live run or readback yet. |
