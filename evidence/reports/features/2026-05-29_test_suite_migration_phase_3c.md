...
| `WindMatrixEnvironment` runtime launch/cleanup helpers | migrated 2026-05-31 (Phase 3D) | `launch()` and `cleanup()` now call plugin-owned `runtime.py` (no `run_matrix.*`/`run_one.*`). See `evidence/reports/features/2026-05-31_test_suite_migration_phase_3d.md`. |
| `WindMatrixEnvironment` assert_ready (heartbeat/vehicle-readiness) | later-phase blocker, outside Phase 3C/3D | Still calls `legacy.run_one_module()` for heartbeat, vehicle-readiness, and slot-timeout. Phase 3E owns removal. |
