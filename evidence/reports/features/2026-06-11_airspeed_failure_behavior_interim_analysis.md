...
| Experiment | Raw root (var/runs/) | Accepted |
| --- | --- | --- |
| Sweep +10..+90, −10..−50 (14 bins) | `airspeed_failure_behavior_ratio_{p10..p90,m10..m50}_2026060{8,9}T*Z` | 3 each (42) |
| Sweep +100 | `airspeed_failure_behavior_ratio_p100_20260608T154448Z` | **2** (attempt_002 failed pre-injection; not hidden, not counted) |
| Pulse ladder | `airspeed_failure_behavior_ratio_bias_pulse_p10_to_p130_headwind_20260610T075155Z` | 1 |
| Ramp +100 | `airspeed_failure_behavior_ratio_bias_ramp_p10_to_p100_headwind_20260610T111134Z` | 1 |
| Ramp +200 | `airspeed_failure_behavior_ratio_bias_ramp_p10_to_p200_headwind_20260610T122611Z` | 1 |

`.BIN` logs exist for all three within-flight experiments. An abandoned ramp
root from 2026-06-09 (no manifest) is excluded. Full path list:
...
- **Addendum (2026-06-11, BIN re-derivation):** the sweep's one-shot
  ...
  *with* detection, flying largely on synthetic airspeed between re-enable
  ...
  `var/analysis/airspeed_failure_ratio_sweep_bin_20260611T000000Z/`. The
...
2. Sweep tables in section 5 are MAVLink-proxy artifacts
   ...
   (`var/analysis/airspeed_failure_ratio_sweep_bin_20260611T000000Z/`)
...
- Curated package (this report's artifact set):
  `evidence/curated_logs/airspeed_failure_behavior_2026-06-11/`
  (`manifest.json` lists SHA-256 + `var/` source for all 27 files;
  `raw_data_index.md` lists all raw roots and exclusions).
- Raw run roots and `.BIN` paths: section 4 and `raw_data_index.md`.
- Raw analysis packages: `var/analysis/airspeed_failure_*` (7 directories,
  2026-06-09..11).
- Lane architecture: `docs/architecture/airspeed_failure_lane.md`.
- Operations runbook: `docs/operations/airspeed_failure_runbook.md`.
- Design decisions: ADR-0006..ADR-0011;
  `governance/runbooks/features/airspeed_failure_behavior/`.
- Phase 2 measurement-smoke acceptance (prior gate):
  `var/runs/airspeed_failure_behavior_20260606T164050810132Z/` (raw),
  recorded in `.ai/current.md` and the feature runbook.
- Checks run for this report (2026-06-11): 27/27 unit tests
  (`tests.unit.test_airspeed_failure_phase1`), `compileall` clean,
  `make doctor` PASS, plus a scripted re-verification of artifacts/readback/
  wind/reset across all 47 accepted attempts.
