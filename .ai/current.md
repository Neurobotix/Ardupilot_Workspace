...
- `scripts/ops/capture_round.sh` decodes raw tlogs into working output under
  ...
  curated artifact. This bootstrap workspace still has no root Git commit, so
  the evidence is not yet Git-tracked.
...
- `workspace_next` root commit: `UNKNOWN`; the repository has no `HEAD` commit.
