...
| BIN finalization, BIN collection, `run_analysis`, `build_run_summary`, wind success classification | `plugins/wind_matrix/analyzers.py` via `WindMatrixAnalyzer` and `WindMatrixVerdictPolicy` | Analysis and wind verdict semantics are plugin-owned; framework cleanup closes and flushes raw logs before staged analyzers run. | Cleanup/flush ordering, partial verdict, non-accepted failure statuses, and staged manifest tests. | Analyzer still delegates to legacy helpers; heavy analysis is not unit-executed. |
| Generic staged orchestration | `core/attempt_runner.py` `StagedStrategy` | Framework owns stage ordering: prepare/launch/readiness, stimulus/control/monitor, cleanup, analyzer/verdict, terminal persistence. Compatibility strategies retain their historical pre-cleanup body. | `test_staged_strategy_calls_stages_in_expected_order`; cleanup success/failure/interrupt tests. | Strategy order is generic; wind after-takeoff remains blocked in staged mode. |
