...
from sim_ard_gaw.campaigns.wind_world import (  # noqa: E402
    SdfWindError,
    read_world_wind,
    write_world_wind,
)
...
class WindWorldSafetyTests(unittest.TestCase):
    ...
    def test_wind_verification_parser_and_policy(self) -> None:
        parsed = run_one.parse_wind_echo(
            "linear_velocity { x: 4.0 y: 8.0 z: 0.0 }\nenable_wind: true"
        )
        self.assertTrue(run_one.STRICT_WIND_ECHO_VERIFY)
        self.assertTrue(run_one.wind_echo_matches(parsed, 4.0, 8.0))
        self.assertFalse(run_one.wind_echo_matches(parsed, 8.0, 4.0))
        self.assertFalse(run_one.wind_echo_matches(
